...
export const Subjects: React.FC = () => {
  ...
  const handleLaunch = (courseId: string) => {
  ...
  return (
    <Shell>
      <div>
        <div className="mb-12">
          <h1 className="text-3xl md:text-5xl font-bold text-slate-800 dark:text-white mb-4 font-headline">What will you <span className="text-indigo-600 font-headline">master</span> today?</h1>
          <div className="flex items-center gap-3 overflow-x-auto pb-4 no-scrollbar">
            <button className="px-6 py-2 bg-indigo-600 text-white rounded-full font-semibold whitespace-nowrap">All Courses</button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {CATALOG_DATA.map(course => {
            const masteredCount = 2; // Mocked progress value
              ...
              <div key={course.id} className="bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 p-8 rounded-[24px] flex flex-col group hover:shadow-md transition-all duration-300">
                ...
                <div className="mt-auto flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="relative w-12 h-12">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                        ...
                        <circle className="text-indigo-600" cx="18" cy="18" fill="none" r="16" strokeDasharray="100" strokeDashoffset={100 - completionPct} strokeLinecap="round" strokeWidth="3" stroke="currentColor" />
                      </svg>
                      <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-indigo-600">{completionPct}%</span>
                    </div>
                    <div>
                  ...
                  <button
                    ...
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 active:scale-95"
                  >
                    Start Journey
